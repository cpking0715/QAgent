...
第 5 节下必须含 `### 5.1 测试层级`。
同步写出 `{output_dir}/test-requirements.drawio`（diagrams.net / VS Code 打开）。`qagent check` 不因缺少导图失败。已有需求可用 `qagent mindmap test-requirements.md -o test-requirements.drawio`。
...
回复用户时列出：产物路径（test-requirements.md、test-requirements.drawio、test-plan.md、risk.md、coverage-matrix.md、testcases.md、qa-review.md、testcases.xlsx）、用例总数、P0/P1/P2 统计、需求覆盖、需求假设清单（如有）。
