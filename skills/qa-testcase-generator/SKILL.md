...
6. **总量控制**：数量以覆盖为准（矩阵每行 ≥1 条、每个 R ≥1 条正向、有边界定义的 R 必出边界/异常用例）；复杂系统（需求条目 ≥15 或多 PDF）约 30~80 条，简单功能约 15~40 条——区间仅作自查参考，明显偏离时检查是否漏测或冗余，禁止为凑数增删用例；需要精简时优先砍 LOW 风险对应的 P2 用例
<!-- /qagent:gen:case_count -->
