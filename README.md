...
| 方式 | 入口 | 适合谁 |
|------|------|--------|
| **A. Web 服务（推荐）** | `qagent serve` | 内网多人：分段确认、SSE 实时进度、产物预览 / AI 审阅 / 本地打开、对话修订 |
...
每段产物也可「本地打开」离线修改后继续；后端等价于 `POST /api/jobs/{id}/run {"from": "test_plan", "stop_after": "coverage_matrix"}`，
CLI 等价于分段执行 `qagent run 需求.md --from auto`（自动从第一个缺失产物的步骤续跑，已有产物不会被覆盖）。
...
3. 生成中状态卡显示 **7 步步骤条**（需求→方案→风险→矩阵→用例→评审→导出：绿勾完成、蓝色脉冲进行中、红叉失败），日志可展开全部；点发送键位置的**停止**可终止（开了 `llm.stream` 时秒级生效，AI 审阅同样可终止）。
4. 右侧**产物抽屉**：下拉分「输入文档 / 生成产物」两组，Markdown 按格式渲染（表格/代码块/勾选框），pdf/docx 自动抽取文本预览；抽屉宽度与审阅区高度可拖拽调整并记忆。产物可在线编辑保存，输入文档点击对话中的文件卡片即可预览。
5. **AI 审阅**：抽屉里任何文件点「审阅」，系统带上交叉参考材料（审用例带矩阵、审方案带需求）输出 总评 / 主要问题（含位置与建议）/ 遗漏与风险 / 快速改进清单，结果落在对话流里，可直接接着说「按审阅第 2 条改」。
6. 交付卡片**点击即预览**；「本地打开」弹出**打开方式选择器**——列出本机声明支持该格式的应用（系统默认置顶、记住上次选择），选中即用对应应用打开文件。
7. 更多菜单：重新整跑 / 补齐缺失产物（保留已有）/ 从方案、风险、矩阵任一环重生成（带覆盖确认）/ 只重生成用例。删除与重跑确认都是页内弹窗，不打断操作。
8. 左侧任务栏可拖宽，任务按**今天 / 昨天 / 更早**分组，条目显示状态、用例数、产物数与创建时间。
9. 完成后可继续对话修订（例如「给登录补异常用例」）；「新对话」开始下一个任务，多个任务可并行。
...
| 任务一直「生成中」、日志不再更新 | 多半是服务被重启、进程没了；点「补齐缺失产物（保留已有）」从断点继续，或「重新整跑」 |
