...
| 方式 | 入口 | 适合谁 |
|------|------|--------|
| **A. Web 服务（推荐）** | `qagent serve` | 内网多人：浏览器上传、看进度、下载、对话修订 |
...
2. 服务会自动建任务并整跑流水线；左侧可看历史任务状态。
3. 完成后下载产物；可点「产物预览」看 Markdown。思维导图优先下载 `test-plan.opml` 导入飞书。
4. 在输入框继续说修订意见（例如「给登录补异常用例」）。
5. 「新对话」开始下一个任务；历史项悬停 × 可删除。
6. 多个任务可以并行跑，执行中也可切换查看。
...
或直接说：「根据 PRD 生成测试方案和测试用例」。

Skill 负责人在回路；`qagent run` / `qagent serve` 负责全自动。
