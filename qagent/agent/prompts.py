...
def _user_supplement_hint(source_text: str) -> str:
    if "## 测试需求" not in source_text:
        return ""
    return """
【用户测试范围说明】源文档含用户提供的测试需求/范围章节，把它当作**范围约束**而不是内容上限：
- 必测 / 不测 / 测试类型的取舍以用户为准；用户写明「不测」的模块与未要求的类型（如性能、压力、兼容）不得写入第 3~6 节覆盖要点，也不得在第 8 节打 ✓，被排除项在第 10 节注明「按用户范围排除 xxx」。
- **范围之内必须详尽**：即使用户只写了一句范围（如「主流程和接口」），第 3~7 节清单仍必须逐模块、逐 API、逐条业务规则地从 PRD 与设计文档中提取，宁多勿漏——用户的范围说明只决定「测不测」，不决定「写多细」。
- 若该章节本身已含完整清单（编号要点/表格），以其为基础，再补齐文档中遗漏的模块与规则。
"""
...
5. 用户未提供详细测试需求（只有范围说明或完全没写）时，第 3~7 节必须从 PRD 与设计文档**逐条提取**：每个必测模块覆盖 正常/边界/异常 三类要点，设计文档出现的每个 API 都进第 4 节，禁止因用户范围写得短就把清单写薄
6. 只输出 test-requirements.md 正文
"""
    return SYSTEM, user
